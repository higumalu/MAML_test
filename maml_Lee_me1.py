...
def meta_task_data(task_num = 2):
  ...
  for t in range(task_num):
    ...
    x = pd.read_excel('dataset.xlsx', sheet_name=t, usecols=[0])
    y = pd.read_excel('dataset.xlsx', sheet_name=t, usecols=[1])
    
    x = x.values.tolist()
    x = list(np.array(x).flatten())
    x = list(np.expand_dims(x, axis=0))
    
    y = y.values.tolist()
    y = list(np.array(y).flatten())
    y = list(np.expand_dims(y, axis=0))
    ...
    label.append('task_'+str(t))
    
  return total_x, total_y, label

x, y, z = meta_task_data()


def test_meta_task_data(task_num = 2):
    total_x = []
    total_y = []
    label = []
    
    
    x = pd.read_excel('dataset.xlsx', sheet_name=2, usecols=[0])
    y = pd.read_excel('dataset.xlsx', sheet_name=2, usecols=[1])
    ...
    total_x = x
    total_y = y
    label.append('task_'+str(2))
    
    plot_x = [np.array(list(map(float,x)))]
    plot_y = []
    for t in range(task_num):
        plot_y.append(np.array(y)) 
